defmodule Tz.PeriodsBuilder do
  ...
  defp offset_diff_from_prev_period(zone_line, local_offset, prev_period) do
    ...
    prev_total_offset =
      prev_period.std_offset_from_utc_time + prev_period.local_offset_from_std_time

    total_offset - prev_total_offset
  end

  defp build_periods_for_zone_line(zone_line, offset, _mode, prev_period)
    ...
    period_from =
      if zone_line.from == :min do
        ...
        add_to_and_convert_date_tuple(
          {prev_period.to.wall, :wall},
          offset_diff,
          zone_line.std_offset_from_utc_time,
          offset
        )
      end
  ...
  defp build_periods_for_zone_line(zone_line, rules, mode, prev_period) when is_list(rules) do
    ...
    if mode == :dynamic_far_future do
      ...
      rules
      |> filter_rules_for_zone_line(
        zone_line,
        prev_period,
        if(prev_period == nil, do: 0, else: prev_period.local_offset_from_std_time)
      )
  ...
  defp filter_rules_for_zone_line(
         ...
         zone_line,
         prev_period,
         prev_local_offset_from_std_time,
         filtered_rules \\ []
       )

  defp filter_rules_for_zone_line(rules, %{from: :min, to: :max}, _, _, _), do: rules

  defp filter_rules_for_zone_line([], _zone_line, _, _, filtered_rules),
    do: Enum.reverse(filtered_rules)
  ...
  defp filter_rules_for_zone_line(
      ...
      cond do
        ...
        zone_line.from == :min || rule.to == :max ->
          ...
          rule_from =
            convert_date_tuple(
              rule.from,
              prev_period.std_offset_from_utc_time,
              prev_local_offset_from_std_time
            )
    ...
    if is_rule_included do
      filter_rules_for_zone_line(
        ...
        zone_line,
        prev_period,
        rule.local_offset_from_std_time,
        [rule | filtered_rules]
      )
    else
      filter_rules_for_zone_line(
  ...
  defp trim_zone_rules([first_rule | tail_rules] = rules, zone_line, prev_period) do
    rules =
      if rule_starts_before_zone_line_range?(
           zone_line,
           first_rule,
           if(prev_period == nil, do: 0, else: prev_period.local_offset_from_std_time)
         ) do
  ...
  defp do_build_periods_for_zone_line([rule | rest_rules], zone_line, prev_period, periods) do
    offset_diff =
      offset_diff_from_prev_period(zone_line, rule.local_offset_from_std_time, prev_period)
      ...
      case prev_period do
        ...
        %{to: :max} ->
          convert_date_tuple(
            rule.from,
            zone_line.std_offset_from_utc_time,
            prev_period.local_offset_from_std_time
          )
    ...
    if period_from != :min && period_to != :max &&
         period_from.utc_gregorian_seconds == period_to.utc_gregorian_seconds do
      raise "logic error"
    end
  ...
  defp convert_date(
         ndt,
         standard_offset_from_utc_time,
         local_offset_from_standard_time,
         :wall,
         :utc
       ) do
    NaiveDateTime.add(
      ndt,
      -1 * (standard_offset_from_utc_time + local_offset_from_standard_time),
      :second
    )
  end
  ...
  defp convert_date(
         ndt,
         _standard_offset_from_utc_time,
         local_offset_from_standard_time,
         :wall,
         :standard
       ) do
  ...
  defp convert_date(
    ...
    NaiveDateTime.add(
      ndt,
      standard_offset_from_utc_time + local_offset_from_standard_time,
      :second
    )
  end
